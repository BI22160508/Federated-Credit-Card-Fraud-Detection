...
ENABLE_DP = False # true to enable it
CLIP_NORM = 1.0
...
def main():
    ...
    df_final.to_csv("exp3_full_vs_topk_dp_all_runs_dp.csv", index=False)
    df_summary.to_csv("exp3_full_vs_topk_dp_mean_std_dp.csv")

    # per-round export
    if round_rows:
        pd.DataFrame(round_rows).to_csv("exp3_full_vs_topk_dp_per_round_logs_dp.csv", index=False)
    ...
    full_src = f"exported_models/exp3_full_seed_{int(best_full['seed'])}.npz"
    topk_src = f"exported_models/exp3_topk_seed_{int(best_topk['seed'])}.npz"

    full_dst = "exported_models/exp3_best_fullfl.npz"
    topk_dst = "exported_models/exp3_best_topkfl.npz"

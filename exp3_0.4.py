...
CLIP_NORM = 1.0
NOISE_MULTIPLIER = 0.4  # std = NOISE_MULTIPLIER * CLIP_NORM
...
def main():
    ...
    df_final.to_csv("exp3_full_vs_topk_dp_all_runs_dp_0.4.csv", index=False)
    df_summary.to_csv("exp3_full_vs_topk_dp_mean_std_dp_0.4.csv")

    # per-round export
    if round_rows:
        pd.DataFrame(round_rows).to_csv("exp3_full_vs_topk_dp_per_round_logs_dp_0.4.csv", index=False)
    ...
    full_src = f"exported_models/exp3_full_seed_{int(best_full['seed'])}_0.4.npz"
    topk_src = f"exported_models/exp3_topk_seed_{int(best_topk['seed'])}_0.4.npz"

    full_dst = "exported_models/exp3_best_fullfl_0.4.npz"
    topk_dst = "exported_models/exp3_best_topkfl_0.4.npz"
